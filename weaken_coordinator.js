...
export async function main(ns) {
  ...
  while (true) {
    ...
    let neededThreads = Math.ceil(
      (currentSecurity - targetSecurity) / weakenEffect
    );
    let totalRamNeeded = weakenScriptSize * neededThreads;
    common.log(
      `Reducing ${server.hostname} to security level ${targetSecurity} will require ${neededThreads} threads, using ${totalRamNeeded}GB`
    );
    ...
    common.log(
      `Spawned ${runningThreads} threads of weaken across ${runningHosts} hosts. Awaiting completion.`
    );
